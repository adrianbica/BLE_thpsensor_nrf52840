...
i2c_bus::~i2c_bus(void)
{
    NRF_LOG_INFO("Called i2c_bus destructor.");
}
...
ret_code_t i2c_bus::write(uint8_t *buffer, uint8_t len)
{
    ret_code_t      err_code;
    m_xfer_done = false;
    err_code = nrf_drv_twi_tx(&m_twi, i2c_dev_addr, buffer, len, false);
    return err_code;
}

ret_code_t i2c_bus::read(uint8_t *buffer, uint8_t len)
{
    ret_code_t      err_code;
    m_xfer_done = false;
    err_code = nrf_drv_twi_rx(&m_twi, i2c_dev_addr, buffer, len);
    return err_code;
}
