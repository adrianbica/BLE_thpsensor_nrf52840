...
THPSensor::THPSensor(void) {
    ...
    state = STATE_INIT;
    sub_state = 0;
//    StateMachineEntry t = {STATE_INIT, &init};
//    sm.push_back(t);
    //sm = {{STATE_INIT, init},{STATE_INIT, init}};
}
...
void THPSensor::init(void) {
    ...
    case 1:
        if (i2c_bus::GetInstance().transfer_ready()) {
            i2c_bus::GetInstance().read(cmd, 1);
            sub_state = 2;
            Timer::GetInstance().ArmTimer(SENSOR_TIMER);
        } else {
            checkAnswerTimeout();
...
void THPSensor::measure(void) {
    ...
    case 2:
        if (i2c_bus::GetInstance().transfer_ready()) {
            ...
            // update BLE
            H.humidity = Humidity;
            err = pTHPService->Update(&H);
            P.pressure = Pressure;
            err = pTHPService->Update(&P);
            T.temperature = Temperature;
            err = pTHPService->Update(&T);
            sub_state = 0;
            Timer::GetInstance().ArmTimer(SENSOR_TIMER);
           /* NRF_LOG_INFO("---");
            NRF_LOG_INFO("Temperature: %ld", Temperature);
            NRF_LOG_INFO("Pressure:    %ld", Pressure);
            NRF_LOG_INFO("Humidity:    %ld", Humidity); */
            NRF_LOG_INFO("Error: %ld", err);
        } else {
            checkAnswerTimeout();
        }
        break;
